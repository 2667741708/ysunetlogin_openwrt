...
6. 兼容有线网关对浏览器请求标识的要求，以及页面脚本形式的校园认证跳转；
   跳转只提取校园 HTTPS 地址，不执行页面脚本，并核对返回的出口 IP。
...
所有命令、别名、输入格式和退出码见 [命令大全](命令大全.md)。
...
`service` 必须为 `0`、`1`、`2` 或 `3`（字符串或整数）。该入口会核实当前账号和运营商，
显式连接时可自动下线不匹配的旧会话。不要把含真实密码的 JSON 加入 Git。

## 验证范围

本机 iYanDa / WLAN 已在系统代理开启时通过网卡绑定直连检测，并读到中国移动在线。
D321 Windows 服务器在停用借网后，已通过自身物理有线网卡完成校园网 0 认证与实际服务核验。
