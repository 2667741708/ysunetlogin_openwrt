...
2.1.3 起，桌面管理器会在查询、设备下线或让目标机器登录运营商之前先执行查询机器资格
预检。“校园网账号”页也提供“检查资格”按钮。预检会确认目标机器能够访问校园网认证环境
并取得 auth1 `sessionId`；不满足时会停止后续操作并提示“需要先实现访问校园局域网”，
避免把使用其他网络访问互联网或只能 SSH 到校内主机误判为已具备校园网认证上下文。
