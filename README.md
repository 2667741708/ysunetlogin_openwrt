...
远端 `netlogin.py` 的目录。桌面程序包含三个页面：

- **一键连接**：选择 SSH 主机后自动带出该主机的默认校园网账号，点击一次完成认证。
  也可以查询当前状态，或让所选服务器当前校园网会话下线，再切换账号/运营商重新登录。
...
- **校园网账号**：保存多个账号、密码和默认运营商。密码通过 Windows DPAPI 加密，
  ...
  设备或全部设备执行下线。
- **SSH 主机**：只读导入当前 Windows 用户 `~/.ssh/config` 中的 `Host` 别名；
...
密码、默认运营商和“某台主机默认使用哪个账号”的映射保存在桌面程序配置中。只有在“SSH 主机”页
...
```powershell
.\release_windows.ps1 -Version 2.0.0
```
