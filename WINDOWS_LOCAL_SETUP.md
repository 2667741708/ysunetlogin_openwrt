...
已加入最新源码，见 [更新日志](RELEASE_NOTES.md) 和 [命令、安装及配置方法](CAMPUS_DIRECT_SETUP.md)。
...
如果本机已经使用同一账号和运营商在线，程序不会重复认证。用户明确点击“使用此账号连接本机”时，如果检测到当前账号或运营商不一致，或者无法确认当前校园网会话属于所选配置，程序会自动下线旧会话，再使用指定配置重新连接，不再要求先手动点击“下线本机”。网络自愈心跳仍采用保守策略：外网可达但校园网门户暂时无法识别账号时先保持现状，避免无人值守时误踢健康连接。

2.1.4 的校园网探测和登录请求会绕过 Windows 系统代理、PAC 以及代理环境变量，因此普通的系统代理即使失效也不应阻断校园 Wi-Fi 认证。若使用 Clash、sing-box 等软件的 TUN/透明代理模式，流量可能在更低层被接管；此时仍需在代理规则中将 `auth.ysu.edu.cn`、`auth1.ysu.edu.cn`、`10.11.0.1` 和校园网段设置为直连。
...
选择为 4090。2.1.4 会优先把已有的 `c201-4090` 连接设为默认位置，也可以随时
改回本机或其他服务器。
