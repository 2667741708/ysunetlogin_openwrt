...
class LocalTargetTests(unittest.TestCase):
    ...
    def test_local_login_uses_selected_account_and_operator(
        ...
        netlogin_class.return_value.login.return_value = (True, '认证成功')
        ...
        netlogin_class.return_value.login.assert_called_once_with(
            user='20260001', pwd='plain-password', type='1')
        unprotect_secret.assert_called_once_with('dpapi-value')

    @mock.patch('desktop_gui.unprotect_secret', return_value='plain-password')
    @mock.patch('desktop_gui.Netlogin')
    def test_local_login_switches_wrong_account_before_connecting(
            self, netlogin_class, unprotect_secret):
        first = mock.Mock()
        second = mock.Mock()
        first.current_status.return_value = {
            'internetOnline': True,
            'summary': {
                'online': True,
                'userId': 'someone-else',
        ...
        first.logout.return_value = (True, '下线成功')
        second.login.return_value = (True, '认证成功')
        netlogin_class.side_effect = [first, second]
